...
import test_wdef_rows_05  # import test.py to get mAP after each epoch
...
last = wdir + 'last_wdef_rows_05_30_mixed.pt'
best = wdir + 'best_wdef_rows_05_30_mixed.pt'
results_file = 'results_wdef_rows_05_30_mixed.txt'
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.ablation:
                ...
                if ni%10==0:
                    retain_size_col = imgs_w
                    retain_size_rows = imgs_h
                
                assert imgs_h == imgs_w, print('Different image shapes')
                imgs, _ = random_mask_batch_one_sample(imgs, ni, retain_size_rows, device, reuse_noise = True)
                ...
                targets = gt_labels_no_loop_export_rows.labels_ablation(targets, imgs_h, imgs_w, ni, retain_size_col, retain_size_rows, device)
        ...
        if not opt.notest or final_epoch:  # Calculate mAP
            ...
            results, maps = test_wdef_rows_05.test(cfg,
                                      data,
                                      batch_size=batch_size,
                                      imgsz=imgsz_test,
                                      model=ema.ema,
