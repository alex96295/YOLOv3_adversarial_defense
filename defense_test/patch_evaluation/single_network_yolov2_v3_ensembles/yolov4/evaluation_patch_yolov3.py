...
patch_results_obj = bb.io.load('det_coco', './json_files/yolov4_patches/proper_patched_obj.json', class_label_map={0: 'person'})
#patch_results_cls = bb.io.load('det_coco', './json_files/yolov3_patches/patch_cls_results.json', class_label_map={0: 'person'})
#patch_results_cls_obj = bb.io.load('det_coco', './json_files/yolov3_patches/patch_obj_cls_results.json', class_label_map={0: 'person'})
#patch_results_random_noise = bb.io.load('det_coco', './json_files/yolov3_patches/random_results.json', class_label_map={0: 'person'})
...
plt.gcf().suptitle('YOLOv4, dataset:INRIA')
plt.gca().set_ylabel('Precision')
plt.gca().set_xlabel('Recall')
plt.gca().set_xlim([0, 1.05])
plt.gca().set_ylim([0, 1.05])
plt.gca().legend(loc=4)
plt.savefig('./pr-curves/single_patches_vs_single_network/yolov4_patch/yolov4_patch.eps')
plt.savefig('./pr-curves/single_patches_vs_single_network/yolov4_patch/yolov4_patch.png')
